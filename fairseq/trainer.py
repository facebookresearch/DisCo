...
class Trainer(object):
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        # update num_updates in the model
        # we need this for curriculum learning
        # TODO: later merge it into the model
        setattr(self.model.decoder, '_num_updates', self.get_num_updates())
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            self.model.eval()
            self.criterion.eval()
            # no curriculum learning for validation
            # we need this for curriculum learning
            # TODO: later merge it into the model
            setattr(self.model.decoder, '_num_updates', None)
