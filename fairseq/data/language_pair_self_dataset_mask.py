...
class LanguagePairSelfDatasetMask(FairseqDataset):
    ...
    def __init__(
        ...
        self.seed = seed
        self.full_masking = full_masking
    ...
    def _make_source_target(self, source, target):
        ...
        min_num_masks = 1
        if self.full_masking:
            min_num_masks = 100000
        ...
        if self.dynamic_masking: 
            ...
            if self.skip_eos:
                if min_num_masks < len(dec_source)-1:
                    sample_size = self.random.randint(min_num_masks, len(dec_source))
                else:
                    sample_size = len(dec_source)-1

                if self.mask_range:
                    start = self.random.randint(len(dec_source) - sample_size)
                    ind = list(range(start, start + sample_size))
                else:
                    ind = self.random.choice(len(dec_source)-1 , size=sample_size, replace=False)
                
                dec_source[ind] = self.tgt_dict.mask()
                dec_target[ind] = dec_target_cp[ind]
                # dec_target is all pads except the masked tokens (not just actual pads but all unmasked tokens). Do not compute the loss for non-masked tokens.
            else:
